...
async function main() {
  ...
  if (communityFilter) {
    console.log(`Filtering communities: ${communityFilter}`)
  }
  if (dryRun) {
  ...
  for (const filename of jsonFiles) {
    ...
    // Only process communities with geo_definition
  ...
  if (communities.length === 0) {
    console.log(communityFilter ? `No communities found matching: ${communityFilter}` : "No communities with geo_definition found")
    return
  }

  console.log(`Found ${communities.length} communit${communities.length === 1 ? 'y' : 'ies'} with geo_definition\n`)
  ...
  for (const { filename, filepath, data } of communities) {
    console.log(`\n${"=".repeat(60)}`)
    console.log(`Community: ${data.name} (${data.code || "no code"})`)
    ...
    // Dissolve geometries using three-stage approach
    ...
    try {
      ...
      for (const geom of unifiedGeometries) {
        if (geom.type === "MultiPolygon") {
          const flattened = flatten({
            type: "Feature" as const,
            geometry: geom,
            properties: { community: data.name },
          })
      ...
      console.log(`  Stage 2: Dissolving ${flattenedFeatures.length} polygons into community boundary...`)

      // Stage 2: Dissolve all polygon features together
      // Note: Don't use propertyName - we want to dissolve ALL features together
      // Without propertyName, dissolve returns a MultiPolygon for disjoint regions
      ...
      if (dissolved && dissolved.features.length > 0) {
        ...
        // For disjoint regions, it returns MULTIPLE Polygon features
  ...
  console.log(`\n${"=".repeat(60)}`)
  console.log(`📊 Summary:`)
  console.log(`  Total communities processed: ${processedCount}`)
  ...
  if (dryRun) {
    ...
    console.log(`\n✅ Community GeoJSON generation complete!`)
  }
}
