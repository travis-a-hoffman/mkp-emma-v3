...
async function main() {
  ...
  try {
    ...
    console.log("Fetching MKP area zipcodes from database...")
    ...
    const rows = await conn.query(`
      SELECT postal_code as zipcode,
             ...
             latitude,
             longitude
      FROM connect_civicrm.civicrm_geocoder_zip_dataset
           INNER JOIN connect_civicrm.civicrm_state_province ON civicrm_geocoder_zip_dataset.state_code = civicrm_state_province.abbreviation AND country_id = 1228
           LEFT OUTER JOIN connect_civicrm.\`State, Area, Community, County, Zip\` ON civicrm_geocoder_zip_dataset.postal_code = zip
      WHERE zip IS NULL AND state_code NOT IN ('PR', 'VI', 'AS')
      ORDER BY state_code, zipcode
    `)
    ...
    const outputDir = path.join(__dirname, "data", outputHostname, "mkp-area-zipcodes")
