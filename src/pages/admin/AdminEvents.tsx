...
import type { Event, EventType, EventWithRelations } from "../../types/event"
...
import AdminEventSummaryView from "./AdminEventSummaryView"
import AdminEventEditView from "./AdminEventEditView"
import AdminEventDetailsView from "./AdminEventDetailsView"
...
export default function AdminEvents() {
  ...
  const [editFormData, setEditFormData] = useState<Record<string, Partial<Event>>>({})
  const [savingItems, setSavingItems] = useState<Set<string>>(new Set())
  const [showArchived, setShowArchived] = useState(false)
  const [archivingItems, setArchivingItems] = useState<Set<string>>(new Set())
  const [archiveConfirmation, setArchiveConfirmation] = useState<{ isOpen: boolean; event: EventWithRelations | null }>(
    {
      isOpen: false,
      event: null,
    },
  ...
  const [participantModalEventId, setParticipantModalEventId] = useState<string | null>(null)
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch events
      const eventsResponse = await fetch(`/api/events?active=${!showArchived}`)
  ...
  const startEditing = (event: EventWithRelations) => {
  ...
  const openParticipantModal = (eventId: string) => {
    setParticipantModalEventId(eventId)
    setParticipantModalOpen(true)
  }

  const handleParticipantPersonSelect = (person: PersonType | null) => {
    if (person && participantModalEventId) {
      const event = events.find((e) => e.id === participantModalEventId)
      if (event) {
        // Add to potential participants list by default
        const updatedPotentialParticipants = [...event.potential_participants]
        if (!updatedPotentialParticipants.includes(person.id)) {
          updatedPotentialParticipants.push(person.id)

          // Update local state only - save will happen when user clicks Save button
          const updatedEvent = {
            ...event,
            potential_participants: updatedPotentialParticipants,
          }
          setEvents((prev) => prev.map((e) => (e.id === participantModalEventId ? updatedEvent : e)))

          setEditFormData((prev) => {
            if (prev[participantModalEventId]) {
              return {
                ...prev,
                [participantModalEventId]: {
                  ...prev[participantModalEventId],
                  potential_participants: updatedPotentialParticipants,
                },
              }
            }
    ...
    setParticipantModalEventId(null)
  }
    ...
    if (primaryLeaderModalEventId) {
      ...
      setEditFormData((prev) => ({
        ...
        [primaryLeaderModalEventId]: {
          ...
          primary_leader_id: person?.id || null,
        },
      }))
  ...
  const openArchiveConfirmation = (event: EventWithRelations) => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-background to-muted">
        ...
        <div className="p-6 pt-20">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center justify-center py-12">
              <Loader2 className="w-8 h-8 animate-spin mr-2" />
              <span className="text-lg">Loading events details...</span>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background to-muted">
      ...
      <div className="p-6 pt-20">
        <div className="min-w-4xl max-w-6xl mx-auto">
          ...
          <div className="mb-8 flex justify-between items-center">
            <p className="text-gray-600">Manage events, scheduling, and participant registration</p>
          </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <div>
                ...
                <CardDescription>Manage events and their comprehensive details</CardDescription>
              </div>
              ...
              {loadingData ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">
                    {showArchived ? "No archived events found" : "No active events found"}
                  </h3>
                  <p className="text-gray-500 mb-4">
                    {showArchived ? "No archived events found." : "Get started by creating your first event."}
                  </p>
                  {!showArchived && (
                            ...
                            {isExpanded && (
                              <div className="border-t bg-card p-4">
                                {isEditing ? (
                                  ...
                                  <AdminEventDetailsView event={event} people={people} />
                                )}
                              </div>
                            )}
                          </CardContent>
      ...
      <EmmaPersonModal
        ...
        onPersonSelect={handleParticipantPersonSelect}
      ...
      <Dialog open={archiveConfirmation.isOpen} onOpenChange={closeArchiveConfirmation}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={archiveEvent} className="bg-red-600 hover:bg-red-700">
              Archive Event
            </Button>
          </DialogFooter>
        </DialogContent>
