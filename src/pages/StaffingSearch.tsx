...
export default function StaffingSearch() {
  ...
  const loadAllUpcomingEvents = async () => {
    setLoading(true)
    try {
      const [eventsResponse, nwtaEventsResponse] = await Promise.all([
        fetch("/api/events?published=true&upcoming=true"),
        fetch("/api/nwta-events?published=true&upcoming=true"),
      ])

      const eventsResult = await eventsResponse.json()
      const nwtaEventsResult = await nwtaEventsResponse.json()

      const allEvents: EventWithRelations[] = []
      ...
      // Add NWTA events
      if (nwtaEventsResult.success && nwtaEventsResult.data) {
        allEvents.push(...nwtaEventsResult.data)
      }

      // Sort combined results by date (earliest first)
      allEvents.sort((a, b) => {
        const dateA = a.participant_schedule?.[0]?.start ? new Date(a.participant_schedule[0].start) : new Date(0)
        const dateB = b.participant_schedule?.[0]?.start ? new Date(b.participant_schedule[0].start) : new Date(0)
  ...
  const searchEvents = async (term: string) => {
    ...
    try {
      const [eventsResponse, nwtaEventsResponse] = await Promise.all([
        fetch(`/api/events?published=true&search=${encodeURIComponent(term)}`),
        fetch(`/api/nwta-events?published=true&search=${encodeURIComponent(term)}`),
      ])
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-center text-3xl font-bold mb-6">Staffing Opportunities</h1>
          ...
          {loading && (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mt-2">
                {hasSearched ? "Searching Staffing Opportunities..." : "Loading Staffing Opportunities..."}
              </p>
            </div>
          )}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                              ...
                              <div className="flex items-center space-x-1">
                                ...
                                <span>
                                  {event.staff_capacity} slots •{" "}
                                  {Math.max(0, event.staff_capacity - (event.committed_staff?.length || 0))} open
                                </span>
                              </div>
                            </div>
                          </div>
          ...
          {!loading && events.length === 0 && !hasSearched && (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground max-w-md mx-auto">
                No upcoming staffing opportunities are currently available. Check back later for new events or use the
