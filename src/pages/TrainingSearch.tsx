...
export default function TrainingSearch() {
  ...
  const loadAllUpcomingEvents = async () => {
    setLoading(true)
    try {
      const [eventsResponse] = await Promise.all([fetch("/api/events?published=true&upcoming=true")])
  ...
  const searchEvents = async (term: string) => {
    ...
    setHasSearched(true)
    try {
      const eventsResponse = await fetch(`/api/events?search=${encodeURIComponent(term)}&published=true&active=true`)
      const eventsResult = await eventsResponse.json()
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-center text-3xl font-bold mb-6">Training Opportunities</h1>
          ...
          {loading && (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mt-2">Searching Training Opportunities...</p>
            </div>
          )}
          ...
          {!loading && events.length === 0 && !hasSearched && (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground max-w-md mx-auto">
                No upcoming training opportunities are currently available. Check back later for new events or use the
